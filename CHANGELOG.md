...
- Replace plain YouTube `<iframe>` with the YouTube IFrame Player API (`window.YT.Player`) so native YouTube controls receive touch events on iPhone / mobile Chrome
- Remove the `mobileTapLayer` overlay when the active moment is a YouTube video, allowing the YouTube player to handle taps directly
- Add real-time playback progress and duration tracking for YouTube items via `YT.Player` event listeners and a 200 ms poll; eliminates the incorrect 10-second text timer that was used as a placeholder
- Add `progress` and `duration` props to `MomentCard`; active cards now render a thin blue progress bar that fills as the moment plays back

### ci
- Add `pr-preview.yml` — builds every project per-PR with correct base URLs, commits preview assets to `main`, and posts preview links as a PR comment
- Add `pr-cleanup.yml` — removes per-PR preview assets from `main` when a PR is closed
- Add `workflow_run` trigger to `deploy.yml` so Pages re-deploys after PR preview commits land on `main`
