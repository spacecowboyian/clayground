...
### feat(fit-gang-bingo)
- **New project** — a back-seat road-trip bingo game for spotting Honda Fits (Vite + React + Tailwind v4 + Gearhead). One printable US Letter sheet holds four 4×4 cards in a 2×2 grid. All 16 squares fill a card exactly, so a single Fisher-Yates shuffle randomizes the square order and the Free Space / Hot Rod placement together; each of the four cards is shuffled independently. Screen chrome is dark per the repo's dark-first tokens; the cards themselves are white for economical printing, and print CSS drops the chrome, forces `print-color-adjust: exact` so swatches survive, and pins the sheet to exactly one page. `Regenerate` reshuffles all four cards, `Print` opens the print dialog. Registered in `docs/projects.json`.
- **Colour pool culled to what's actually spottable** — Honda shipped six blues on the US Fit (Tidewater, Celestial, Blue Sensation, Vortex, Aegean, Reflection), indistinguishable from a moving car; they collapse into Light Blue and Dark Blue. Blue Raspberry (BG59M, 2012–13) survives as its own square because it is not blue but **teal** — the chip hue is sampled from photographs of real BG59M cars rather than inferred from the name, as are the other chips (ImportArchive reference photos). The three surviving blues are separated on lightness *and* saturation, since equal-value chips read as one colour through a car window regardless of hue. Final pool: White, Black, Silver, Dark Gray, Red, Orange, Yellow, Light Blue, Dark Blue, Blue Raspberry, Purple.
- **Generation squares** — GD3 (2007–08), GE8 (2009–13), GK5 (2015–20) join the grid, filling it back to 16 after the colour cull without reintroducing hard-to-call colours. Each shows the car itself, since the shape is what you actually call from a moving car; the artwork is greyscale so a generation square can never read as a colour square. Year ranges are live text in the artwork's red, not baked into the image, for the same legibility reason as the Hot Rod caption. There is no green or brown US-market Fit.
- **Four Hot Rod cars, one per card** — the blue / orange / purple / light-blue artwork is cycled by card index (not shuffled, so a player can find their own card at a glance). "Any modification applies" is rendered as live text under the car rather than baked into the artwork: at this cell size the printed caption collapsed into an unreadable smudge, and as text it also matches every other cell's structure (visual on top, label in the white band).
- **Grid restyled to match the artwork** — header art on every card, plus the Free Space and Hot Rod tiles. Hairline black rules (the grid container showing through 1.5px gaps, so interior lines never double up), sticker-style paint chips with black keylines, and heavy italic labels, replacing thin gray borders and generic caps that read as a spreadsheet next to the brush-lettered art. Generation squares carry the artwork's red on their year ranges (5.10:1 on white). Every cell keeps a white lower band in the same position: the mark zone, verified by simulating pen X marks so an X still reads over Black and Dark Blue.
