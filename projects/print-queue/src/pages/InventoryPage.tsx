...
import { Button, Dialog, Accordion, Card, SearchField } from '@gearhead/ui'
                              ...
                              <tr key={model.id} className="border-b border-[var(--border)] hover:bg-[var(--secondary)] transition-colors">
                                ...
                                <td className="px-4 py-3 text-right">
                                  <div className="flex items-center justify-end gap-1">
                                    {model.model_url && (
                                      <a href={model.model_url} target="_blank" rel="noopener noreferrer" className="p-1.5 rounded hover:bg-[var(--accent-blue-light)] text-[var(--muted-foreground)] hover:text-[var(--accent-blue)] transition-colors" title="Open model">
                                        <ExternalLink className="w-4 h-4" />
                                      </a>
                                    )}
                                    <button onClick={() => setEditModel(model)} className="p-1.5 rounded hover:bg-[var(--accent-orange-light)] text-[var(--muted-foreground)] hover:text-[var(--accent-orange)] transition-colors" title="Edit">
                                      <Pencil className="w-4 h-4" />
                                    </button>
                                    <button onClick={() => setDeleteModelTarget(model)} className="p-1.5 rounded hover:bg-[var(--accent-red-light)] text-[var(--muted-foreground)] hover:text-[var(--destructive)] transition-colors" title="Delete">
                          ...
                          <div key={model.id} className="p-4 space-y-1">
                            <div className="flex items-start justify-between gap-2">
                              <div>
                                ...
                                {model.number && (
                                  <p className="text-xs text-[var(--muted-foreground)]">#{model.number}</p>
                                )}
                                {model.description && (
                      ...
                      return (
                        ...
                        >
                          ...
                          <div>
                            <div className="flex items-center gap-2 flex-wrap">
                              {model.number && (
                                <span className="text-xs text-[var(--muted-foreground)]">#{model.number}</span>
                              )}
                              <p className="font-medium text-[var(--foreground)]">{model.name}</p>
                              {model.self_created && (
                  ...
                  {models.length > 6 && !showAllDefault && (
                    <p className="text-xs text-center text-[var(--muted-foreground)]">
                      ...
                      <button
                        ...
                        onClick={() => setShowAllDefault(true)}
                      >
                        Show all
                      </button>
